...
with conn:
	...
	print(f"The last sample is: '{inf_start_sample}'")
...
def text_command(message):
	...
	if message.text == m.first_btn2:
		...
		with conn:
			...
			for row in data:
				bot.send_message(message.chat.id, f'"{row[0]}": {row[1]} - {row[2]}', reply_markup=m.back_markup)
				bot.register_next_step_handler(message, get_project)
...
def get_project(message):
	...
	### Удаление проекта
	if message.text == 'Удалить последний проект':
		user_id = [int(message.from_user.id)]
		
		with conn:
			cursor.execute("SELECT id, user_id FROM projects_list ORDER BY id DESC LIMIT 1")
			data = cursor.fetchall()
			for row in data:
				if user_id[0] == int(row[1]):
					with conn:
						cursor.execute("DELETE FROM projects_list WHERE id=?", (row[0],))
						conn.commit()
						bot.send_message(message.chat.id, 'Ваш последний проект успешно удален', reply_markup=m.first_markup)
						bot.register_next_step_handler(message, text_command)
						return
				else:
					bot.send_message(message.chat.id, 'Последний проект зарегестрирован не Вами', reply_markup=m.first_markup)
					bot.register_next_step_handler(message, text_command)
					return
	...
	if project_data_list.count(",") != 1:
		...
		### Проверка на сответсвие кол-ва проб числу
		try:
		...
		### Проверка на сответсвие кол-ва проб числу определенного размера
