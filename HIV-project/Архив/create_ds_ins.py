...
with open ('records\mutations_DS\ins.json', 'r') as file:
    data = json.load(file)
    ...
    for sequence in keys_list:
        ...
        RT_lst_for_nums = [ins_list_RT, drug_mut_RT, mut_list_RT, RT_random_mutations]
        ...
        x = 2
        while x == 2:
            RT_m_pos = random.sample (range(1,100), 16)
                ...
                for num in RT_m_pos:
                    ...
                    for m in dct.keys():
                        ...
                        if int(re.findall(r'\d+', m)[0]) == num:
                            ...
                            # вставка делеций
                            if dct == ins_list_RT:
                                if len(list(ran_ins_list_RT.keys()))<5: 
                                    val = dct[m]
                                    if m not in ran_ins_list_RT.keys():
                                        ran_ins_list_RT[m] = []
                                    ran_ins_list_RT[m] += val
                                    RT_m_pos.remove(num)
                        ...
                        if ran_drug_mut_RT != {} and ran_mut_list_RT != {} and R_random_mutations_RT != {} and ran_ins_list_RT!= {}:
                            x +=1
                        elif ran_drug_mut_RT == {} or ran_mut_list_RT == {} or R_random_mutations_RT == {} or ran_ins_list_RT == {}:
                            continue
        ...
        ran_ins_list_RT_lst = []
        for key in ran_ins_list_RT.keys():
            for item in ran_ins_list_RT[key]:
                ran_ins_list_RT_lst += [str(key+item)]  
        data[sequence]['Reverse Transcriptase']["Insertions"] = ran_ins_list_RT_lst

        # выбор мутации, поиск кодона в рефе, выбор кодона мутации
        turn = [ran_drug_mut_PR_lst, ran_mut_list_PR_lst, ran_drug_mut_RT_lst, ran_mut_list_RT_lst, R_random_mutations_PR_lst, R_random_mutations_RT_lst]
        ...
        # всавка инсерции
        for insertion in ran_ins_list_RT_lst:
            mut_number = int(re.findall(r'\d+', mut)[0])
            f_codon_number = mut_number*3-3
            data[sequence]['Reverse Transcriptase']["RT sequense"] = data[sequence]['Reverse Transcriptase']["RT sequense"][:f_codon_number] + 'NNN' + data[sequence]['Reverse Transcriptase']["RT sequense"][f_codon_number:]


        # Переименовывание последовательности
        new_name = str(sequence[-8:-1]) + str(sequence[-1])
        if ran_ins_list_RT_lst in turn:
            for deletion in ran_ins_list_RT_lst:
                new_name += '_'
                new_name += deletion
        data[new_name] = data.pop(sequence)
    
new = json.dumps(data, indent=2)
with open ('records\mutations_DS\ins.json', 'w') as load_file:
        load_file.write(new)
print('DONE')
